...
class MaliciousEvaluator:
    ...
    def evaluate_malicious_detection(self, normal_data_dir, malicious_csv_path):
        ...
        # Calculate detection rates at specific FPR thresholds
...
def save_result(results, model_type, strategy, save_path, personalized=False):
    os.makedirs(save_path, exist_ok=True)  # create folder if not exists
    if personalized:
        file_path = os.path.join(save_path, f"personalized_evaluation_{model_type}_deepsvdd_{strategy}_result.json")
    else:
        file_path = os.path.join(save_path, f"evaluation_{model_type}_deepsvdd_{strategy}_result.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--strategy", type=str, default="fedavg", choices=["fedavg", "fedprox"], help="Oneclass model type")
    ...
    try:
        ...
        save_result(results=results, model_type=args.model_type, strategy=args.strategy, save_path=args.output)
